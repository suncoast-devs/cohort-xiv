 ...
 *    // 3. make that word uppercase
 *    // 4. add that word to a new array
 *    // 5. see if this word is the last word in the array
 *    // 6. if not, go to the next word and go back to step 2
 *    // 7. If it is, we are done, return the new array
 * }
 */
...
const yelling = array => {
  ...
  function wordTransformerFunction(currentWord) {
    let upperCaseWord = currentWord.toUpperCase()

    return upperCaseWord
  }

  const wordTransformerFunctionVariable = function(currentWord) {
    let upperCaseWord = currentWord.toUpperCase()

    return upperCaseWord
  }

  const wordTransformerArrow = currentWord => {
    let upperCaseWord = currentWord.toUpperCase()

    return upperCaseWord
  }

  let newArray = array.map(wordTransformerArrow)
...
const everyoneIsOdd = array => {
  const isTrue = array.every(isOdd)
...
test('findTheNeedle', t => {
  t.is(findTheNeedle(['one', 'time', 'there was a needle at', 'the market']), 'there was a needle at')
})
